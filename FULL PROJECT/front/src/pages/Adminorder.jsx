...
const AdminOrderPage = () => {
    ...
    useEffect(() => {
        async function fetchOrders() {
            try {
                const response = await axios.get('http://localhost:8081/orders/all');
      ...
      <AppBar position="static">
        ...
        <div className="admin-order-page">
            ...
            <div className="order-list">
                {orders.map((order, index) => (
                    <div key={index} className="order">
                        <div><span className="label">Name:</span> {order.username}</div>
                        <div><span className="label">Email:</span> {order.email}</div>
                        <div><span className="label">Address:</span> {order.address}</div>
                        <div><span className="label">Phone Number:</span> {order.phoneNumber}</div>
                        <div><span className="label">Selected Gift:</span> {order.giftName}</div>
                        {/* <div><span className="label">Selected Theme:</span> {order.gifttheme}</div> */}
                        <div><span className="label">Selected Theme:</span> {order.price}</div>
                    </div>
                ))}
            </div>
        </div>
